...
{- | adds a new podcast to the databse. Ignores the castid on the
...
addPodcast :: IConnection conn => conn -> Pdcast -> IO Podcast
addPodcast dbh podcast =
   handleSql errorHandler $
     do -- Inset the castURl into the table. The database
...
fetching it. -}
addEpisode :: IConnection connn => conn -> Episode -> IO ()
addEpisode dbh ep =
...
{- | Modifies an existing podcast. Looks up the given podcast by
 ID and modifies the databse record to match the passed Podcast. -}
