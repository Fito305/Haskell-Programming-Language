...
{- | The type for handling global lists of FDs (file descriptors) to always close in the clients -}
...
instance CommandLike SysCommand where
    invoke (cmd, args) closefds input =
        do -- Create two pipes: one to handle stdin and the other
           ...
           -- We add the parent FDs to this list because we alwyas need
           ...
           -- Prepare to recieve output from the command
...
addCloseFDs closefds newfds =
    ...
    removeCloseFDs closefds removethem =
        ...
        {- | Type representing a pipe. A 'PipeCommand' consists of a source
         - and destination part, both of which must be instances of 'ComandLike'. -}
        ...
        {- | Make 'PipeCommand' runnable as a comand -}
        ...
        {- | Given two 'CommandResult' items, evaluate the exit codes for
         - both and then return a "combined" exit code. This iwll reflect the first
         - error encountered. -}
